...
public class CrosswalkToScene11Trigger : MonoBehaviour
{
    [Header("Refs")]
    ...
    [Header("Condition")]
    public bool requireScene10 = true;
    ...
    {
        if (!other.CompareTag("CityCat") && !other.transform.root.CompareTag("CityCat")) return;
        ...
        // ✅ Scene10일 때만
        if (requireScene10 && cue.actMgr.Current != ActId.Scene10) return;

        // 1) Scene11 진입
        cue.actMgr.SwitchActImmediate(ActId.Scene11);

        // 2) Act 반영
        cue.SetAct(cue.IndexFromAct(ActId.Scene11));
